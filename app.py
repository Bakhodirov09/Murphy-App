...
async def link_btn(chat_id):
    link_button = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text='Murphy-Homework Web-Site', web_app=WebAppInfo(url=f'https://12b3492d0594.ngrok-free.app?chat_id{chat_id}'))
            ]
        ]
    )
    return link_button
...
✊ GOOD LUCK...!
"""
    await message.answer(text=text, reply_markup=await link_btn(message.chat.id), protect_content=True, parse_mode='HTML')
